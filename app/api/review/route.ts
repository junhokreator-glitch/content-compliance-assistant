...
- excerpt: 입력 원문의 언어 그대로, 실제 존재하는 문구를 인용합니다.
- suggestions: 입력 원문과 같은 언어로 작성하고, 콘텐츠에 바로 대체해 넣을 수 있는 완성된 문구만 담습니다. 여러 안을 제시할 때도 각 배열 항목은 하나의 완성된 문구여야 하며, "또는", "등", 안내 문구, 괄호 설명 등을 덧붙이지 않습니다. 안이 여러 개면 배열 항목을 늘립니다.
- reason, basis: 검수 담당자가 한국어로 읽으므로 입력 언어와 무관하게 한국어로 작성합니다.
...
- CONDITIONAL: 객관적 근거(조사 기관·표본 수·조사 방법·임상 결과 등)를 함께 제시하거나 보조 문구를 덧붙이면 그대로 쓸 수 있는 경우.
  예) 수치·비율 주장, 순위·점유율·선택률 주장, 검증·입증 주장, 시술 결과 예시 제시
...
5. 대체 표현(suggestions)은 원문의 마케팅 의도와 핵심 의미를 유지하는 범위에서만 제안하며, 원문에 없는 사실이나 효능을 새로 추가하지 않습니다. 동의·권리 문제인 경우에는 표현 대신 필요한 조치를 제시합니다.
...
10. revisedContent는 findings의 suggestions를 개별적으로 이어 붙인 것이 아니라, 원문 전체를 놓고 다시 자연스럽게 작성한 하나의 완성된 글입니다. 문장 연결(조사, 어미)이 어색해지지 않도록 필요하면 문장 구조 자체를 바꿉니다. 리스크가 없던 부분은 원문 그대로 유지합니다. findings가 비어 있으면 원문과 동일하게 반환합니다.
   category가 "콘텐츠 권리·동의" 또는 "환자 정보"인 finding은 문구 자체가 문제가 아니라 사전 동의·확인 같은 별도 절차가 빠진 것이므로, 해당 부분의 원문 문장은 고치지 않고 그대로 둡니다. 그 finding의 suggestions(필요한 조치)를 revisedContent 본문 안에 지시문·안내문 형태로 써 넣지 않습니다. 이런 finding만 있고 "마케팅 표현"·"의료기기 광고" finding이 없다면 revisedContent는 원문과 동일합니다.`;
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          category: {
            type: "string",
            enum: ["마케팅 표현", "의료기기 광고", "콘텐츠 권리·동의", "환자 정보"],
          },
    ...
    revisedContent: {
      type: "string",
      description:
        "마케팅 표현·의료기기 광고 리스크를 반영해 다시 쓴, 원문 전체 분량의 완성된 수정본. 콘텐츠 권리·동의/환자 정보 findings는 문구 수정 대상이 아니므로 반영하지 않는다.",
    },
  },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const 원문보존확인 = 조치대상_findings.every((f: { excerpt: string }) =>
      결과.revisedContent?.includes(f.excerpt),
    );
